...
public class Scrabble {
	...
	public static int wordScore(String word) {
		...
		for (int i = 0; i < word.length(); i++){
			...
			sum += SCRABBLE_LETTER_VALUES[value];
		}
		sum *= word.length();
	...
	public static String createHand() {
		String randomStr = MyString.randomStringOfLetters(HAND_SIZE - 2);
		randomStr = MyString.insertRandomly('a', randomStr);
		randomStr = MyString.insertRandomly('e', randomStr);
		return randomStr;
	}
	...
	public static void playHand(String hand) {
		...
		while (hand.length() > 0) {
			...
			if (input.equals(".")){
				...
				String word = input;
				boolean isWordValid = MyString.subsetOf(word, hand);
				boolean isWordExists = isWordInDictionary(word);

				if (!isWordValid) {
					System.out.println("Invalid word. Try again.");
					continue;
				} else if (!isWordExists) {
